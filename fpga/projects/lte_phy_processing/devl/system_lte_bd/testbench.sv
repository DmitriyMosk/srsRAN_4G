...
module tb_system_lte_bd;
    ...
    logic                            i_data_valid_i1;
    logic                            i_data_valid_q1;

    logic signed [`HW_ADC_WIDTH-1:0] o_data_i1;
    logic signed [`HW_ADC_WIDTH-1:0] o_data_q1;
    logic                            o_data_valid_1;
    logic                            o_pss_valid;
    logic [$clog2(`LTE_PSS_COUNT)-1:0] o_dbg_pss_idx;
    logic [31:0]                     o_dbg_shift;
    ...
    system_lte_bd #(
        .LTE_CORR_FS(FS_HZ),
        .LTE_CORR_LANES(K_LANES),
        .LTE_PSS_TD_LEN(PSS_TD_LEN),
        .LTE_TARGET_FS(FS_HZ),
        .LTE_SUBFRAME_SPS(SUBFRAME_SPS),
        .LTE_BUF_CAP(BUF_CAP)
    ) dut (
        .i_clk(clk),
        .i_rst(rst),
    ...
    always @(negedge clk) begin
        ...
        if (rst) begin
            i_data_valid_i1 <= 1'b0;
            i_data_valid_q1 <= 1'b0;
            i_data_i1       <= '0;
            i_data_q1       <= '0;
            ce_cnt          <= 0;
            acc             <= 0;
            ...
            if (fire) begin
                ...
                if (!ok_i || !ok_q) begin
                    ...
                    i_data_valid_i1 <= 1'b1;
                    i_data_valid_q1 <= 1'b1;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            frame_fill_count       <= 0;
            frame_q_wr             <= 0;
        end else if (dut.u_system_lte_bd.u_lte_pss_detector.fifo_do_write) begin
    ...
    always @(posedge clk) begin
        if (o_pss_valid) begin
            ...
            if ($isunknown({o_dbg_pss_idx, o_dbg_shift, o_dbg_mag_pss0, o_dbg_mag_pss1, o_dbg_mag_pss2}))
                $fatal(1, "Decision bus contains X/Z while o_pss_valid=1");

            detect_frame_base_abs = frame_base_queue[detect_count];
            abs_detect_shift      = detect_frame_base_abs + o_dbg_shift;
            abs_expected_shift    = EXPECT_FIRST_SHIFT + (EXPECT_PERIOD * detect_count);

            $display("[%0t] VALID #%0d: PSS%0d rel_start0=%0d frame_base_abs=%0d abs_start0=%0d data_valid=%0b mags={%0d,%0d,%0d}",
                     $time, detect_count, o_dbg_pss_idx, o_dbg_shift, detect_frame_base_abs,
            ...
            if ((detect_count + 1) == EXPECT_DETECTIONS) begin
                $display("TB PASSED: system_lte_bd decision bus is synchronous with o_pss_valid");
                $finish;
            end
        end
    end
